...
public class PaletteBox: NSObject, NSCoding {
    ...
    public init(key: Int, color: CGColor){
        self.key = key
        self.color = color
    }
    public convenience required init?(coder aDecoder: NSCoder) {
        guard
            ...
            let key = aDecoder.decodeObject(forKey: kKey) as? Int else {
...
public class NESPalette: NSObject, PaletteProtocol, NSCoding {
    public var type: PaletteType = .nes
    public var size = 4 // Number of colors in a NES tile
    private var _palette: [(colorCode: Int, color: CGColor)]? = nil
    public var palette: [(colorCode: Int, color: CGColor)]  {
        get {
            if let p = _palette {
                return p
            }
    ...
    public convenience init?(data: Data) {
        guard data.count == 4,
            let color0 = NESAvailableColors[data[0]],
            let color1 = NESAvailableColors[data[1]],
            let color2 = NESAvailableColors[data[2]],
            let color3 = NESAvailableColors[data[3]] else {
            return nil
        }
        self.init()
        let palette = [(Int(data[0]), color0),
                       (Int(data[1]), color1),
                       (Int(data[2]), color2),
                       (Int(data[3]), color3)]
    ...
    func randomColor() -> (Int, CGColor) {
        let randomNumber = Int(arc4random_uniform(UInt32(NESAvailableColors.count)))
        
        let key = Array(NESAvailableColors.keys)[randomNumber]
        guard let color = NESAvailableColors[key] else {
            return (0, CGColor(red: 0.329411764705882, green: 0.329411764705882, blue: 0.329411764705882, alpha: 1.0))
        }
        return (Int(key), color)
    }
    ...
    public convenience required init?(coder aDecoder: NSCoder) {
        ...
        var palettes: [(colorCode: Int, color: CGColor)] = []
    ...
    public func encode(with aCoder: NSCoder) {
        ...
        self._palette?.forEach({ (tuple: (key: Int, color: CGColor)) in
...
public class GeneralNESColorPalette: PaletteProtocol {
    ...
    public var palette: [(colorCode: Int, color: CGColor)] = GeneralNESColorPalette.nesColors()
    public init() {  }
    public static func nesColors() ->  [(colorCode: Int, color: CGColor)] {
...
fileprivate var NESAvailableColors: [UInt8: CGColor] = [
...
fileprivate func sortedNesColors() -> [(Int, CGColor)] {
    var ret = [(Int, CGColor)]()
    nesKeyOrder.forEach { (key: UInt8) in
        // TODO: Must remove force unwrap
        ret.append((Int(key), NESAvailableColors[key]!))
    }
