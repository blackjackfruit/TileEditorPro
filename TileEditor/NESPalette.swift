...
protocol PaletteProtocol: class {
    ...
    var palette: [(Key, Value)] { get set }
    
    func count() -> Int
    func set(position: Int, key: Key, value: Value)
}
...
class NESPalette: PaletteProtocol {
    ...
    fileprivate var _palette: [(String,CGColor)]? = nil
    var palette: [(String,CGColor)] {
        get {
            if let p = _palette {
                return p
            }
    ...
    let keys: [String]
    init() {
        keys = Array(nesColors.keys)
    }
    
    func randomColor() -> (String, CGColor) {
        let randomNumber = Int(arc4random_uniform(UInt32(keys.count)))
        let key = keys[randomNumber]
        guard let value = nesColors[key] else {
                return ("00", nesColors["00"]!)
        }
        return (key, value)
    }
}
...
"21":CGColor(red: 0.298039215686275, green: 0.603921568627451, blue: 0.925490196078431, alpha: 1.0),
...
"2d":CGColor(red: 0.235294117647059, green: 0.235294117647059, blue: 0.235294117647059, alpha: 1.0),
